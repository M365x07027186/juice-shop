...
describe('OAuthComponent', () => {
  ...
  it('will create regular user account with base64 encoded reversed email as password', fakeAsync(() => {
    ...
    component.ngOnInit()
    expect(userService.save).toHaveBeenCalledWith({ email: 'test@test.com', bW9jLnRzZXRAdHNldA==: 'bW9jLnRzZXRAdHNldA==', passwordRepeat: 'bW9jLnRzZXRAdHNldA==' })
  }))
  ...
  it('logs in user even after failed account creation as account might already have existed from previous OAuth login', fakeAsync(() => {
    ...
    component.ngOnInit()
    expect(userService.login).toHaveBeenCalledWith({ email: 'test@test.com', bW9jLnRzZXRAdHNldA==: 'bW9jLnRzZXRAdHNldA==', oauth: true })
  }))
